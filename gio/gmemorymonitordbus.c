...
lmm_start_fallback_backends (GMemoryMonitorDBus *dbus)
{
  ...
  g_debug ("LowMemoryMonitor was notfound, starting fallback backends.");
...
#else
  /* waiting for the DBus name to appear, if the system isn't Linux or the
  ...
  if (result)
    {
      ...
      while (g_variant_iter_loop (iter, "s", &str))
        {
          if (!g_strcmp0 (str, "org.freedesktop.LowMemoryMonitor"))
            {
